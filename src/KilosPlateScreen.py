...
class KilosPlateScreen(Screen):
    """The kilos plate guesser screen.

    Args:
        Screen (kivy.uix.screenmanager.Screen): A kivy Screen object.
    """
    total = StringProperty("20")
    ...
    def updateTotal(self):
        ...
        """
        self.total = str((int(self.red)*25+int(self.blue)*20+int(self.yellow)*15+int(self.green)*10+int(self.ten)*5+int(self.five)*2.5+int(self.two)*1.25+int(self.one)*0.5+int(self.half)*0.25)*2+20)
        return
